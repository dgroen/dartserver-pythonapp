...
## Full Disaster-Recovery Drill (2026-08-24)
Ran the complete cycle for real: brought up an isolated `postgres` (renamed/reported to avoid colliding with the real `darts-*` stack, otherwise using the actual `docker-compose-wso2.yml` unmodified) -> seeded a distinctive marker row in `dartsdb` and a marker file in `wso2is_data` -> backed up with `helpers/backup_docker_volumes.sh` -> **destroyed everything** (`docker-compose down -v`, all containers and volumes gone) -> restored with the now-fixed `helpers/restore_docker_volumes.sh` -> brought the stack back up. **Both markers survived intact**, postgres booted healthy afterward, and the real `darts-*` stack was completely unaffected throughout.

Found and fixed two more real bugs along the way:
- **`restore_postgres_dump()`'s fixed `sleep 10`** before attempting the SQL-dump restore was too short and caused the restore to fail outright on the first drill attempt ("database system is starting up"). Replaced with a proper `pg_isready` wait loop (up to 300s), matching the pattern already used elsewhere in the migration tooling.
- **A significant, separate finding**: a **fresh** `postgres_data` volume loading the full official WSO2 DDL (~200 tables/indexes) reliably exceeded postgres's default 60-second shutdown timeout while checkpointing during its internal temp-server-to-real-server transition, causing the container to **exit fatally mid-init** — this reproduced with fsync enabled (the correct, real setting; my earlier synthetic rehearsal had masked this by using `fsync=off`). This is a real risk for **any** first-time provisioning of postgres for WSO2 in test or production, not just this restore drill. Fixed in `docker-compose-wso2.yml` with `PGCTLTIMEOUT: "300"` (the officially-supported postgres image env var for exactly this scenario) and a matching `start_period: 300s` on the healthcheck so it doesn't flap unhealthy mid-init either.

Also observed and since fixed (2026-08-24): `restore_docker_volumes.sh` was running both the raw volume restore *and* the SQL-dump restore unconditionally, even though they cover the same data — the SQL restore threw harmless "already exists"/"duplicate key" errors on top of data the volume restore had already correctly brought back. `restore_postgres_dump()` now skips itself when `postgres_data` was already restored from its own volume backup, falling back to the SQL dump only when that volume backup wasn't available. Verified with a focused re-test.

## Branch/Pipeline Follow-Ups Still Needed (2026-08-24)
This machine has no `gh` CLI and no GitHub token available, so these need to be done by hand:
- Open/merge a PR for `feature/migrate_to_pgsql` into `dev`/`main` — nothing on this branch has been reviewed yet.
- Configure the `production-migration-approval` GitHub Environment (Settings -> Environments -> Required Reviewers) so `.github/workflows/wso2-migration-rehearsal.yml` can actually run — it will otherwise hang waiting for an approval gate that doesn't exist yet.
