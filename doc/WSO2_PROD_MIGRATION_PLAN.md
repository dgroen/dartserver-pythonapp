...
Also observed (pre-existing, not newly introduced, not fixed): `restore_docker_volumes.sh` runs both the raw volume restore *and* the SQL-dump restore unconditionally, even though they cover the same data — the SQL restore threw harmless "already exists"/"duplicate key" errors on top of data the volume restore had already correctly brought back. Not a data-loss risk, just noisy; worth deciding later whether to make the SQL-dump restore conditional/skippable.
