...
def test_games_store_integration():
    ...
    from src.app.app import games_store, active_game_id
    ...
    from src.app.app import start_game
    source = open('src/app/app.py').read()
    assert 'games_store[game_id]' in source, "/api/game/start should update games_store"
    print("✅ Test 2: /api/game/start endpoint updates games_store")
    
    # Test 3: Check that /api/game/new endpoint has games_store logic
    assert source.count('games_store[game_id] = {') >= 3, "Should have games_store updates in at least 3 endpoints"
    ...
    for template_file in [
        ...
        with open(template_file) as f:
            content = f.read()
            assert "'gamemaster' in user_roles or 'admin' in user_roles" in content, \
